...
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 3: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀  
 ...
 art: function getArtistByIndex(array, index) {
  return 'The artist ${index} is ${index.name}';
}
...
function get20s(artists){
  for (var i=0; artists.length; i++){
  if(artists[i].years.split(' - ') > "1900" && data[i].years.split(' - ') <= "1999"){
year.push(artists[i].years);
...
function addArtist(array, id, name, years, genre, nationality, bio){
 ...
 console.log(addArtist(artists, 20, 'Sierra', '1999-', 'web development', 'American', 'Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum'));
...
for (var i=0; artists.length; i++){
  if (artists[i].paintings > 100){
    paint.push(artists[i].paintings);
  }
  return paint;
}
